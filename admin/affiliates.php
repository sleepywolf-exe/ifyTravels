    ...
    if (empty($name) || empty($email) || empty($code)) {
        ...
        if ($exists) {
            ...
            if ($db->execute("INSERT INTO affiliates (name, email, code, status) VALUES (?, ?, ?, 'active')", [$name, $email, $code])) {
                ...
                $error = "Database Error: Failed to create affiliate. Ensure the table exists.";
            }
        }
    }
}
                ...
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <div
                        ...
                        <form method="POST" class="p-6">
                            ...
                            <h3 class="text-lg font-bold leading-6 text-gray-900 mb-4" id="modal-title">Add New
                                Affiliate</h3>
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input type="text" name="name" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border px-3 py-2">
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="code" placeholder="e.g. SUMMER25" required
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm border px-3 py-2 uppercase">
                                    <p class="text-xs text-gray-500 mt-1">Must be unique. Will be converted to
                                        uppercase.</p>
                                </div>
                            </div>
                            ...
                            <div class="mt-6 flex justify-end gap-3">
                                <button type="button" onclick="closeModal()"
                                    class="rounded-md border border-gray-300 bg-white px-4 py-2 text-base font-medium text-gray-700 shadow-sm hover:bg-gray-50 focus:outline-none sm:text-sm">Cancel</button>
                                <button type="submit"
                                    class="rounded-md border border-transparent bg-primary px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-teal-700 focus:outline-none sm:text-sm">Create
                                    Partner</button>
                            </div>
                        </form>
                    </div>
