...
function getMockDestinations() {
    return [
        ['slug' => 'bali-trip', 'updated_at' => date('Y-m-d')],
        ['slug' => 'maldives-honeymoon', 'updated_at' => date('Y-m-d')],
        ['slug' => 'dubai-adventure', 'updated_at' => date('Y-m-d')],
        ['slug' => 'thailand-getaway', 'updated_at' => date('Y-m-d')]
    ];
}

function getMockPackages() {
    return [
        ['slug' => 'bali-5n-6d-package', 'updated_at' => date('Y-m-d')],
        ['slug' => 'kerala-houseboat-special', 'updated_at' => date('Y-m-d')],
        ['slug' => 'manali-volvo-package', 'updated_at' => date('Y-m-d')]
    ];
}
...
try {
    ...
    echo "Database connection failed. Using mock data.\n";
    $hasDb = false;
}
...
foreach ($staticPages as $path => $priority) {
    $url = base_url($path);
    if (!filter_var($url, FILTER_VALIDATE_URL) || strpos($url, 'localhost') !== false) {
...
$pagesXml .= '</urlset>';
file_put_contents($baseDir . '/sitemap-pages.xml', $pagesXml);
...
$destinations = $hasDb ? $db->fetchAll("SELECT * FROM destinations ORDER BY created_at DESC") : getMockDestinations();
...
$destXml .= '</urlset>';
file_put_contents($baseDir . '/sitemap-destinations.xml', $destXml);
...
$packages = $hasDb ? $db->fetchAll("SELECT * FROM packages ORDER BY created_at DESC") : getMockPackages();
...
$pkgXml .= '</urlset>';
file_put_contents($baseDir . '/sitemap-packages.xml', $pkgXml);
...
file_put_contents($baseDir . '/sitemap.xml', $indexXml);
