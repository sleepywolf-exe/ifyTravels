...
<div class="mt-4">
        ...
        <div class="swiper tags-swiper !overflow-visible">
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide !w-auto">
                    <a href="<?php echo $url; ?>" class="block px-5 py-2.5 rounded-full text-sm font-semibold transition-all <?php echo $isActive ? 'bg-primary text-white shadow-lg shadow-primary/30' : 'bg-white text-slate-600 border border-slate-200 active:scale-95'; ?>">
                        <?php echo $tag; ?>
                    </a>
                </div>
                <?php endforeach; ?>
    ...
    <div class="px-4 mb-4 flex items-center gap-2">
        ...
        <a href="<?php echo base_url('mobile/explore.php'); ?>" class="ml-auto text-xs font-bold text-red-500 bg-red-50 px-2 py-1 rounded-full">Clear</a>
    </div>
    <?php endif; ?>
    ...
    <div class="px-4 mb-8">
        ...
        <div class="grid grid-cols-2 gap-4">
            <?php foreach ($destinations as $dest): ?>
                <a href="<?php echo destination_url($dest['slug']); ?>" class="relative h-28 rounded-2xl overflow-hidden shadow-md group">
                    <img src="<?php echo base_url($dest['image_url']); ?>" class="w-full h-full object-cover group-active:scale-110 transition-transform duration-500">
    ...
    <div class="px-4 space-y-6 pb-safe">
        ...
        <?php foreach ($packages as $pkg): ?>
            <a href="<?php echo package_url($pkg['slug']); ?>" class="block bg-white rounded-3xl overflow-hidden shadow-lg border border-slate-100 group">
                <div class="relative h-48">
                    <img src="<?php echo base_url($pkg['image_url']); ?>" class="w-full h-full object-cover transition-transform duration-700 group-active:scale-105">
                    <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                    <div class="absolute top-3 right-3 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold shadow-sm">
                ...
                <div class="p-4 flex justify-between items-center">
                    <div>
                        ...
                        <span class="text-primary font-black text-xl">₹<?php echo number_format($pkg['price']); ?></span>
                    </div>
                    <span class="px-4 py-2 bg-slate-900 text-white text-sm font-bold rounded-full shadow-lg shadow-slate-900/20 group-active:scale-95 transition-transform">View</span>
                </div>
            </a>
        <?php endforeach; ?>
        ...
        <div class="text-center py-10">
            ...
            <a href="<?php echo base_url('mobile/explore.php'); ?>" class="inline-block mt-4 text-primary font-bold">Clear Filters</a>
        </div>
        <?php endif; ?>
    </div>
</div>
